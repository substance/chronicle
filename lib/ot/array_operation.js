...
// This case is reduced by treating the move as composite (deletion+insertion).
function transform_move_move(a, b) {

  // if `a`, `b` move the same element we make `b` the effective change,
  // and disabe the effect of `a`
  if (a.pos === b.pos) {
    ...
    var a_c = new Chronicle.OT.Compound([new ArrayOperation([DEL, a.pos, '']), new ArrayOperation([INS, a.target, ''])]);
    var b_c = new Chronicle.OT.Compound([new ArrayOperation([DEL, b.pos, '']), new ArrayOperation([INS, b.target, ''])]);
    ArrayOperation.transform(a_c, b_c, {check: false, inplace: true});

    a.pos = a_c.ops[0].pos;
    a.target = a_c.ops[1].pos;
    b.pos = b_c.ops[0].pos;
    b.target = b_c.ops[1].pos;
  }
...
var hasConflict = function(a, b) {
  ...
  if ( (a.type === DEL  || a.type === MOV) &&
       (b.type === DEL  || b.type === MOV) ) {
    return (a.pos === b.pos);
  }
  else {
...
ArrayOperation.chain = function(ops) {
  ...
  for (j = 1; j < ops.length; j++) {
    for (i = 0; i < j; i++) {
      var t = ArrayOperation.transform(transformed[i], transformed[j]);
      transformed[j] = t[1];
    }
  }
...
ArrayOperation.position = function(pos, target) {
  var dels = [];
  var ins = {type: INS, pos: target, val: ''};
  var idx;

  for (idx = 0; idx < pos.length; idx++) {
    dels.push({type: DEL, pos: pos[idx], val: ''});
  }

  while (dels.length > 0) {
    var del = dels.shift();
    var mov = new ArrayOperation([MOV, del.pos, ins.pos]);
    for (idx = 0; idx < dels.length; idx++) {
      transform_move_delete(mov.copy(), dels[idx]);
    }
    transform_move_insert(mov.copy(), ins);
  }
};
...
ArrayOperation.Insert = function(pos, val) {
  return new ArrayOperation(["+", pos, val]);
}
...
ArrayOperation.Move = function(pos1, pos2) {
  return new ArrayOperation([">>", pos1, pos2]);
}
