...
ObjectOperation.__prototype__ = function() {
  ...
  this.apply = function(obj) {
    ...
    var prop = (obj.resolve) ? obj.resolve(this.path) : ObjectOperation.resolve(obj, this.path);

    if (this.type === CREATE) {
      if (prop.get() !== undefined) {
        throw new errors.ChronicleError("Value already exists. path =" + JSON.stringify(this.path) + ", in: " + JSON.stringify(obj));
      }

      prop.set(util.clone(this.val));
    }

    else if (this.type === DELETE) {
      // TODO: maybe we could tolerate such deletes
      if (prop.get() === undefined) {
        throw new errors.ChronicleError("Property " + JSON.stringify(this.path) + " not found " + JSON.stringify(obj));
      }
...
ObjectOperation.Property = function(obj, path) {
  this.obj = obj;
  this.path = path;
};

ObjectOperation.Property.__prototype__ = function() {
...
ObjectOperation.Property.prototype = new ObjectOperation.Property.__prototype__();

ObjectOperation.resolve = function(obj, path) {
  return new ObjectOperation.Property(obj, path);
};
