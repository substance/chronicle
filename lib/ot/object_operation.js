    ...
    if (newVals[key] === undefined && obj[key] !== undefined) {
      ...
      if (obj[key] === undefined) {
        ...
        if (typeof oldVal !== typeof newVal) {
          throw new errors.ChronicleError("Can not update values of different types");
        }

        if (_.isString(oldVal)) {
          // compute diff for text (use google diff)
        } else if (_.isArray(oldVal)) {
          // compute diff for array (similar to google diff)
        } else {
          // not yet implemented
          throw new errors.ChronicleError("not supported yet");
        }
