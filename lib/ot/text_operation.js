...
// Converts from a given a sequnence in the format of Tim's lib
...
TextOperation.fromOT = function(ot, str) {
  var ops = []; // atomic ops
  ...
  _.each(ot, function(op) {
    if (_.isString(op)) { // insert chars
      ops.push(TextOperation.Insert(dstPos, op));
      dstPos += op.length;
    } else if (op<0) { // delete n chars
      var n = -op;
      ops.push(TextOperation.Delete(dstPos, str.slice(srcPos, srcPos+n)));
      srcPos += n;
    } else { // skip n chars
      srcPos += op;
      dstPos += op;
...
var Range = function(range) {
  ...
  // control the behaviour when characters are inserted at the boundary
  // if set to true the range will expand
  this.EXPAND_LEFT = range.EXPAND_LEFT || false;
  this.EXPAND_RIGHT = range.EXPAND_RIGHT || false;
};
...
var range_transform = function(range, textOp) {
    // Compound
    ...
    if (textOp.type === DEL) {
      ...
      if ( (pos < start) ||
           (pos === start && !this.EXPAND_LEFT) ) {
        start += l;
      }

      if ( (pos < end) ||
           (pos === end && this.EXPAND_RIGHT) ) {
        end += l;
      }
    }
...
Range.__prototype__ = function() {
  ...
  this.toJSON = function() {
    ...
    if (this.EXPAND_LEFT) result.EXPAND_LEFT = true;
    if (this.EXPAND_RIGHT) result.EXPAND_RIGHT = true;

    return result;
  };

  this.transform = function(textOp) {
    range_transform(this.range, textOp);
  };

};
Range.prototype = new Range.__prototype__();

Range.transform = function(range, op) {
  range_transform(range, op);
}
