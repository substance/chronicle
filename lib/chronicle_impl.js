  ...
  this.merge = function(id, strategy, options) {
    ...
    if (strategy === "mine") {
      ...
      if (sequence.length == 0) {
        ...
        var tmp1 = this.index.diff(head, sequence[0]);
        var tmp2 = this.index.diff(id, sequence[0]);
        var reverts1 = tmp1.reverts();
        var reverts2 = tmp2.reverts();
        var applies1 = tmp1.applies().concat(sequence.slice(1))
        var applies2 = tmp2.applies().concat(sequence.slice(1))
        // and append the rest of the sequence
        // Note: exploiting the knowledge about the data structure
        data[head] = Chronicle.Diff.create(head, reverts1, applies1);
        data[id] = Chronicle.Diff.create(id, reverts2, applies2);
      }
    } else {
    ...
    if (Chronicle.HYSTERICAL && strategy === "manual") {
      try {
        private.apply.call(this, change.id);
        private.revertTo.call(this, head);
      } catch(err) {
...
ChronicleImpl.__private__ = function() {
  ...
  this.applyDiff = function(diff, force) {
    ...
    try {
      ...
      for (var idx = 0; idx < reverts.length; idx++) {
        var id = reverts[idx];
        private.revertTo.call(this, id, force);
    ...
    if (err && idx > 0) {
      ...
      try {
        private.applyDiff.call(this, inverted, force);
      } catch(_err) {
  ...
  this.revertTo = function(id, force) {
    ...
    if (current instanceof Chronicle.Merge) {
      ...
      private.applyDiff.call(this, diff.inverted(), true);
    } else {
  ...
  this.apply = function(id) {
    ...
    if (change instanceof Chronicle.Merge) {
      private.applyDiff.call(this, change.diff[current], true);
      this.versioned.setHead(id);
    } else {
