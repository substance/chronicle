...
ChronicleImpl.__private__ = function() {
    ...
    while(queue.length > 0) {
      ...
      if (source instanceof Chronicle.Merge) {
        ...
        if (this.versioned.hasConflict(a, b)) {
          // TODO: have a conflict resolver
          // automatic: mine, theirs
          // interactive:
          //  TODO: THINK! it sounds like we are missing some concept here.
          //        The merge would need to be interruptible or asynchronous.
          //        What is the greater pain: implementing 'yielding' or asynchonification?
          throw new errors.ChronicleError("Can not rebase: conflicting changes.");
        }
  ...
  this.rebase = function(sourceId, targetId) {
    ...
    if (eliminations.length > 0) {
      rebased = __private__.eliminate.call(this, sourceId, eliminations, {}, tmp_index);
    }
    ...
    while (forwards.length > 0) {
      id = forwards.shift();
      rebased = __private__.rebase0.call(this, id, rebased, tmp_index);
    }
    ...
    while (idx1 < branch.length && idx2 < eliminations.length) {
      ...
      if (id === del) {
        ...
        if (last) {
          ...
          last = __private__.eliminate.call(this, last, id, mapping, tmp_index);
        }
        idx1++; idx2++;
      } else {
        last = id;
