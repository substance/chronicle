...
ChronicleImpl.__private__ = function() {
  ...
  this.eliminate = function(start, del, mapping, index) {
    index = index || this.index;
    ...
    // transform the tree
    rebased = __private__.rebase0.call(this, inverted.id, right.id, mapping, index);
  ...
  this.rebase0 = function(targetId, sourceId, mapping, index) {
    index = index || this.index;
    mapping = mapping || {};
    ...
    while(queue.length > 0) {
      ...
      if (source instanceof Chronicle.Merge) {
        ...
        b_i = new Chronicle.Transformed(targetId, transformed[1],
          (source instanceof Chronicle.Transformed) ? source.original : source.id);
      }
      ...
      for (idx = 0; idx < children.length; idx++) {
        queue.unshift([transformed[0], b_i.id, index.get(children[idx])]);
      }
    }
  ...
  this.rebase = function(sourceId, targetId) {

    // TODO: make sure that the given id is root of a graph.
    // This can be done by a BFS sweep checking that every parent of a merge node
    // can be reached directly from this node.

    // use a temporary index so that the main index stays untouched on errors
    var tmp_index = new Chronicle.TmpIndex(this.index);

    var rebased;

    // if necessary, eliminate all changes to make the change a child
    // of the common root
    var diff = tmp_index.diff(sourceId, targetId);
    var eliminations = diff.reverts().slice(0, -1);
    if (eliminations.length > 0) {
      rebased = __private__.eliminate(sourceId, eliminations, {}, tmp_index);
    }

    // iteratively rebase up the chain
    var id;
    var forwards = diff.applies();
    while (forwards.length > 0) {
      id = forwards.shift();
      rebased = __private__.rebase0(id, rebased, tmp_index);
    }

    // store the rebased changes into the main index
    tmp_index.save(rebased);
  ...
  this.eliminateToSelection = function(branch, sequence, index) {
    ...
    // keeping track of transformed ids
    var mapping = {};

    var idx1 = branch.length-1, idx2 = 0;
    var idx, id, del;
    var last = null;

    // initialize the id mapping
    // Note: it is necessary to keep a mapping between original change
    // ids and new ids of transformed changes.
    for (idx = 0; idx < selection.length; idx++) {
      id = selection[idx];
      mapping[id] = id;
    }
    
    while (idx1 > 0 && idx2 < eliminations.length) {
      id = branch[idx1];
      ...
      if (id === del) {
        ...
        if (last) {
          ...
          last = __private__.eliminate(last, id, mapping, tmp_index);
        }
        idx1--; idx2++;
      } else {
        last = id;
  ...
  this.manualMerge = function(head, id, diff, sequence) {
      ...
      var reverts = diff.reverts();
      var root = _.last(reverts);
      
      var mine = reverts.slice(-1).reverse();
      var theirs = diff.applies();


      var mapping = {};
      __private__.eliminateToSelection(mine, sequence, mapping, tmp_index);
      __private__.eliminateToSelection(theirs, sequence, mapping, tmp_index);
      ...
      for (idx=0; idx<sequence.length; idx++) {
        ...
        // skip un-transformed changes
        if (nextId === sequence[idx]) {
          lastId = nextId;
        } else {
          __private__.rebase0(nextId, lastId, mapping, tmp_index);
        }
      }
      ...
      for (idx=0; idx<sequence.length; idx++) {
        if (mapping[sequence[idx]] !== sequence[idx]) {
