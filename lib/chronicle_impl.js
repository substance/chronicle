...
var ChronicleImpl = function(index, versioned) {
  Chronicle.call(this, index, versioned);
}
...
ChronicleImpl.__prototype__ = function() {
  ...
  this.record = function(change) {
    ...
    if (Chronicle.HYSTERICAL) {
      this.versioned.revert(change);
      this.versioned.apply(change);
    }
  ...
  this.reset = function(id) {
    ...
    if (Chronicle.HYSTERICAL && !this.index.contains(id)) throw new ChronicleError("Invalid argument: unknown change "+id);
  ...
  this.merge = function(id, strategy, options) {
    ...
    if (strategy === "mine") {
      ...
      if (!sequence) throw new ChronicleError("Invalid argument: sequence is missing for manual merge");
...
ChronicleImpl.__private__ = function() {
  ...
  this.applyDiff = function(diff) {
    ...
    if (originalState !== diff.start()) throw new errors.ChronicleError("Diff can not applied on to this state. Expected: "+diff.start()+", Actual: "+originalState);
